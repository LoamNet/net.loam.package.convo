...
namespace Loam.Convo
{
    ...
    public class ConvoBasicDialogUI : MonoBehaviour
    {
        ...
        [SerializeField] private ConvoBasicDialogUIOption _optionTemplate;

        // Internal
        private Stack<ConvoBasicDialogUIOption> _optionPool;
        private List<ConvoBasicDialogUIOption> _optionActive;
        ...
        public void Awake()
        {
            ...
            _optionPool = new Stack<ConvoBasicDialogUIOption>();
            _optionActive = new List<ConvoBasicDialogUIOption>();
        ...
        /// Show self entirely
        /// </summary>
        public void Show(string conversationJSON)
        {
            ConversationSystem.Load(conversationJSON);
            ShowTarget(_self);
        }
        ...
        private void ShowTarget(CanvasGroup target)
        {
            target.alpha = 1;
        ...
        /// </summary>
        /// <param name="target"></param>
        private void HideTarget(CanvasGroup target)
        {
            target.alpha = 0;
        ...
        private void LineUpdated()
        {
            ...
            if (ConversationSystem.TryGetCurrentSpeaker(out string speaker))
            {
                ...
                {
                    HideTarget(_groupSpeakerL);

                    ShowTarget(_groupSpeakerR);
                    _labelSpeakerR.text = speaker;
                }
                else
                {
                    HideTarget(_groupSpeakerR);

                    ShowTarget(_groupSpeakerL);
            ...
            if (ConversationSystem.TryGetCurrentLine(out ConversationSystem.Line main, out IReadOnlyList<ConversationSystem.Line> options))
            {
                if (options != null && options.Count > 0)
                {
                    ...
                    {
                        ConvoBasicDialogUIOption option = GetOption();
        ...
        /// </summary>
        /// <returns></returns>
        private ConvoBasicDialogUIOption GetOption()
        {
            ConvoBasicDialogUIOption option = null;
        ...
        private void PoolOption(ConvoBasicDialogUIOption toPool, int index = -1)
        {
            if (index < 0)
            {
